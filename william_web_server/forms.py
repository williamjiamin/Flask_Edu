...
class RegistrationForm(FlaskForm):
    username = StringField("昵称",validators=[DataRequired(message="虽然英雄无名，但是还请大侠写一个名字呗"),Length(min=2,max=20,message="不好意思，昵称至少要2——20字符之间哦~")])
    email = StringField("邮箱信息",validators=[DataRequired(message="大侠，现在是21世纪，留个邮箱好联系啊，飞鸽传书臣妾做不到啊！"),Email(message="大神，您是穿越过来的吗？email的格式是yourname@188.com这样的~")])
    password = PasswordField("密码",validators=[DataRequired(message="额滴个神啊，大锅您不输入密码怎么注册啊！")])
    confirm_password = PasswordField("再次确认你的密码",validators=[DataRequired(message="额滴个神啊，大锅您不输入密码怎么注册啊！"),EqualTo("password",message="这位高手，别逗我玩了，您两次输入的密码不一样啊！")])
